...
GameBoyAdvanceMMU.prototype.freeze = function () {
  return {
    ram: [
      ...new Uint16Array(
        cloneDeep(this.memory[this.REGION_WORKING_RAM].buffer)
      ),
    ],
    iram: [
      ...new Uint16Array(
        cloneDeep(this.memory[this.REGION_WORKING_IRAM].buffer)
      ),
    ],
  };
};
...
GameBoyAdvanceMMU.prototype.defrost = function (frost) {
  this.memory[this.REGION_WORKING_RAM].replaceData(
    new Uint16Array(frost.ram).buffer
  );
  this.memory[this.REGION_WORKING_IRAM].replaceData(
    new Uint16Array(frost.iram).buffer
  );
};
